...
// atRHS
template <uint Dim>
bool atRHS(const Loop<Dim>& l, const uint& j) {
	uint nj = negNeigh(j,l.size());
	number dzp = DX(l,j,Dim-2);
	number dzn = DX(l,nj,Dim-2);
	number dtp = DX(l,j,Dim-1);
	number dtn = DX(l,nj,Dim-1);
	return (dzp<0.0 && dzn>0.0 && dtp>0.0 && dtn>0.0);
}

// atLHS
template <uint Dim>
bool atLHS(const Loop<Dim>& l, const uint& j) {
	uint nj = negNeigh(j,l.size());
	number dzp = DX(l,j,Dim-2);
	number dzn = DX(l,nj,Dim-2);
	number dtp = DX(l,j,Dim-1);
...
// atRHSDisjoint
template <uint Dim>
bool atRHSDisjoint(const Loop<Dim>& l, const uint& j, const number& beta) {
	uint nj = negNeighDisjoint(j,l.size());
	number dzp = DXDisjoint(l,j,Dim-2,beta);
	number dzn = DXDisjoint(l,nj,Dim-2,beta);
	number dtp = DXDisjoint(l,j,Dim-1,beta);
	number dtn = DXDisjoint(l,nj,Dim-1,beta);
	return (dzp<0.0 && dzn>0.0 && dtp>0.0 && dtn>0.0);
}

// atLHSDisjoint
template <uint Dim>
bool atLHSDisjoint(const Loop<Dim>& l, const uint& j, const number& beta) {
	uint nj = negNeighDisjoint(j,l.size());
	number dzp = DXDisjoint(l,j,Dim-2,beta);
	number dzn = DXDisjoint(l,nj,Dim-2,beta);
	number dtp = DXDisjoint(l,j,Dim-1,beta);
...
template bool atRHS<4>(const Loop<4>& loop, const uint& i);
...
template void PosMinDXMu<2>(const Loop<2>& loop, const uint& i, const uint& mu, uint& posMin, number& currentMin);
template void PosMinDXMuDisjoint<2>(const Loop<2>& loop, const uint& i, const uint& mu, uint& posMin, number& currentMin, const number& beta);
template bool atRHS<2>(const Loop<2>& loop, const uint& i);
template bool atLHS<2>(const Loop<2>& loop, const uint& i);
template bool atRHSDisjoint<2>(const Loop<2>& loop, const uint& i, const number& beta);
template bool atLHSDisjoint<2>(const Loop<2>& loop, const uint& i, const number& beta);
template bool atCoord<2>(const Loop<2>& loop, const uint& mu, const uint& coord, const uint& j);
...
template number DSm<2> (const Loop<2>& l, const Point<2>& p, const uint& loc);
template number KG<2> (const Loop<2>& l);
template number KGMax<2> (const Loop<2>& l);
template number KGMax<2> (const Loop<2>& l, const uint& ex1, const uint& ex2);
template number S0<2> (const Loop<2>& l);
template number S0Disjoint<2> (const Loop<2>& l, const number&);
template number DS0<2> (const Loop<2>& l, const Point<2>& p, const uint& loc);
template number FGamma<2> (const Loop<2>& l);
template number DFGamma<2> (const Loop<2>& l, const Point<2>& p, const uint& loc);
